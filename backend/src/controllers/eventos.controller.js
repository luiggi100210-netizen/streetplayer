...
async function unirseEvento(req, res, next) {
  try {
    ...
    if (evento.creador_id !== req.usuario.id) {
      await pool.query(
        'INSERT INTO notificaciones (usuario_id, tipo, mensaje, referencia_id) VALUES ($1,$2,$3,$4)',
        [evento.creador_id, 'evento', `${req.usuario.username} se unió a "${evento.titulo}"`, id]
      );
    }
...
async function finalizarEvento(req, res, next) {
  try {
    const { id } = req.params;
    ...
    try {
      ...
      // Marcar participantes como asistentes o ausentes
      await client.query(
      ...
      // XP y estadísticas por participante
      ...
      // Registrar calificaciones pendientes (24h)
